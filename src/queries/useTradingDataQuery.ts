...
const useTradingDataQuery = (type: 'price' | 'valume' | 'countmaxmin', currencyKey?: string) => {
  ...
  const fetch = useCallback(async () => {
    let url = ''
    if (type === 'price') {
      // url = `http://122.51.71.147:3001/price/${currencyKey}`
      url = 'http://122.51.71.147:3001/price/btc'
    } else if (type === 'valume') {
      url = `http://122.51.71.147:3001/shadows/${currencyKey}/valume`
    } else {
      url = `http://122.51.71.147:3001/shadows/${currencyKey}/countmaxmin`
    }
    ...
    setData(result.map(item => {
      const value = item.vlaue.map(v => weiToBigNumber(v))

      const low = BigNumber.min.apply(null, value).toNumber()
      const high = BigNumber.max.apply(null, value).toNumber()

      return ({
        ...item,
        open: value[0].toNumber(),
        close: value[1].toNumber(),
        high,
        low,
